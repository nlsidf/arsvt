...
impl Renderer {
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, camera: &Camera, world: &World, items: &[Item], npcs: &[NPC], monochrome_mode: bool) {
        ...
        let mut sprite_order: Vec<(usize, f64, char, Color)> = Vec::new();
        ...
        for item in items {
            ...
            if transform_y > 0.1 && transform_y < 20.0 {
                ...
                if sprite_screen_x > 0 && sprite_screen_x < width as i32 {
                    ...
                    sprite_order.push((sprite_screen_x as usize, transform_y, icon, color));
                }
            }
        }
            ...
            if transform_y > 0.1 && transform_y < 20.0 {
                ...
                if sprite_screen_x > 0 && sprite_screen_x < width as i32 {
                    let icon = match npc.npc_type {
                        crate::entities::NPCType::Wanderer => '🚶',
        ...
        for (screen_x, depth, icon, color) in sprite_order {
            if screen_x < width {
                ...
                if draw_y < height {
                    self.buffer[draw_y][screen_x] = icon;
                    self.color_buffer[draw_y][screen_x] = color;
                }
            }
        }
