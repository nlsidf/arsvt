        ...
        function initTerminal() {
            ...
            try {
                ...
                term.onData(function(data) {
                    ...
                    console.log('Input char codes:', Array.from(data).map(c => c.charCodeAt(0)));
                    ...
                    if (ws && ws.readyState === WebSocket.OPEN) {
                        ...
                        // Windows版本：立即在本地回显输入内容
                        try {
                            term.write(data);
                        } catch (e) {
                            console.error('Failed to echo locally:', e);
                        }
                        
                        // 创建二进制消息：第一个字节是命令类型'0'，后面是数据
                        var msg = new Uint8Array(processedData.length + 1);
                        msg[0] = '0'.charCodeAt(0);  // 命令类型0表示输入数据
                        
                        // 将字符串数据转换为字节数组
                        for (var i = 0; i < processedData.length; i++) {
                            msg[i + 1] = processedData.charCodeAt(i);
                        }
                        
                        console.log('Sending message to server:', msg);
                        console.log('Message content as string:', processedData);
                ...
                ws.onopen = function() {
                    ...
                    if (term.cols && term.rows) {
                        ...
                        try {
                            ...
                            // 创建带有命令字符'{'的完整消息
                            var fullMsg = new Uint8Array(initBytes.length + 1);
                            fullMsg[0] = '{'.charCodeAt(0);  // 命令字符'{'
                            fullMsg.set(initBytes, 1);       // JSON数据
                            ws.send(fullMsg);
                            console.log('Init message sent with command character');
                        } catch (e) {
